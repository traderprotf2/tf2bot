"""
Bundled, static Unusual particle effect name<->id mapping - does NOT need
steam_api_key or any network call, unlike steam_schema.fetch_particle_
name_to_id (main.py's other source for this same data, used to OVERLAY
anything newer this bundle doesn't have yet, when a working steam_api_key
happens to be configured).

Per explicit request: relying solely on a live Steam API fetch meant a
missing/broken steam_api_key silently left EVERY particle effect
unresolved - even a common, long-established one like Blizzardy Storm
(added 2011) - which broke both display (the raw, un-stripped effect
prefix staying attached to the item name) and, more subtly, the
correctness of the alert's particle-effect text. This bundle is the
guaranteed baseline that works regardless of API key status; nothing
here depends on any credential or live request ever succeeding.

Sourced from https://raw.githubusercontent.com/mninc/tf2-effects
(a maintained, community-run reference list), cross-checked against the
official TF2 Wiki's own "Unusual effect ids" reference page. Covers
every standard cosmetic/taunt/weapon Unusual effect from the game's
original 2010 Unusual introduction through Smissmas 2022 (id 3104) -
several hundred effects, comfortably covering the vast majority of
what's actually seen in real trading, since older effects have had far
more time to circulate than brand-new ones. Effects from Summer 2023
onward are NOT included here (no single, current, comprehensively-
IDed public source was found covering that gap at the time this was
compiled) - those still rely on the live schema fetch when available,
same as before this bundle existed.
"""
