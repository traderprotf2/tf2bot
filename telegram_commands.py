...
HELP_TEXT = (
    ...
    "/unknowneffects — список ID эффектов Unusual, которых ещё нет во "
    "встроенной базе (unusual_effects.py) - помогает дополнять базу по "
    "факту наблюдаемой торговли\n"
    ...
    "/memtrace, /memtop, /memtraceoff — включить/посмотреть/выключить "
    "профайлер памяти (tracemalloc), если снова растёт RSS - показывает "
    "точную строку кода, а не догадки\n"
    "/memtrim — вручную попросить glibc вернуть освобождённую, но не "
    "отданную ОС память (malloc_trim) - показывает RSS до/после\n"
    "/help — это сообщение"
)
...
def build_accounts_menu(runtime):
    text = (
        ...
        "Несколько аккаунтов ускоряют: проактивное сканирование (не только "
        "Unusual - любая отслеживаемая категория/качество), "
